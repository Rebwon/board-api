...
include::{snippets}/errors-account/response-fields.adoc[]

예를 들어, 잘못된 요청으로 계정 혹은 게시글을 만들려고 했을 때 다음과 같은 `400 Bad Request` 응답을 받는다.

include::{snippets}/errors-account/http-response.adoc[]
include::{snippets}/errors-posts/http-response.adoc[]
...
계정 리소스는 계정을 만들거나 조회할 때 사용한다.

[[resources-accounts-create]]
=== 계정 생성

`POST` 요청을 사용해서 새 계정을 만들 수 있다.
...
operation::query-posts[snippets='response-fields,curl-request,http-response,links']
...
operation::create-post[snippets='request-fields,curl-request,http-request,request-headers,http-response,response-headers,response-fields,links']
...
operation::get-post[snippets='request-fields,curl-request,http-response,links']
...
operation::update-post[snippets='request-fields,curl-request,http-response,links']